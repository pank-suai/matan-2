...
/ Теорема: Пусть дан ряд с положительными членами $sum^(infinity)_(n = 1) U_n$ и пусть $U_1 >= U_2 => U_3 => dots $,
  ...
  называют производящей, то ряд $sum^(infinity)_(n=1)$ U_n и несобственный интеграл $integral^(infinity)_1 f(x) dif x$ будут
  вести себя одинаково в смысле сходимости.
...
#theorem[
  ...
  / Доказательство: Рассмотрим двойное неравенство
   ...
   $ 0 <= U_n + |U_n| <= 2|U_n| $ для $n = 1, 2, ... $ .
    ...
    Пусть ряд $sum_(n=1)^infinity |U_n|$ сходится, тогда ряд $sum_(n=1)^infinity 2|U_n|$ также будет сходиться, а по признаку сравнения будет сходящимся и ряд
...
При исследовании ряда @un2 на сходимость можно применять все до-статочные признаки сходимости, установленные для знакоположительных рядов.

/ Теорема: Абсолютно сходящийся ряд при любой перестановке его членов остаётся абсолютно
  сходящимся и его сума не изменяется.


/ Замечание: Утверждение теоремы справедливо для любого сходящегося знакопостоянного ряда. Условно сходящиеся ряды этим свойством не обладают.
